...
def update_line(line):
    ...
    if stripline[:len(PRINT)] == PRINT:
        spaces = ' ' * line.find(PRINT)     #if there are spaces before print in line
        content = stripline[len(PRINT)+1 :] #to remove space and taking all after that in line
        return '{}print({})'.format(spaces,content)#retain spaces before and print out the content after print in ()
...
def update_pre_block(pre_block):
    ...
    """
    lines = pre_block.split("\n")        # we get list of lines except without \n
    updated_block = update_line(lines[0])#first line updated
    for line in lines[1 :]:
        updated_block += "\n"            # every updated line of code in new line
        updated_block += update_line(line)# updating line
